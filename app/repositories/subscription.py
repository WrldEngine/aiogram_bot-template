...
class SubscriptionRepository(AbstractRepository):
    _model: Subscriptions
    _session: AsyncSession

    async def create(self, **data) -> Subscriptions:
    ...
    async def update(self, user_id: int, **data) -> Subscriptions:
        async with self._session as session:
            stmt = (
                update(self._model)
                .where(self._model.account_id == user_id)
                .values(**data)
    ...
    async def get_single_record(self, **filters) -> Subscriptions:
        async with self._session as session:
